###
# This file is developed specifically to deal with inhouse sentiment analysis problem. it is having 3 class.
###
import argparse
import json
...
import config
from extensible_classification_framework.src.models import (ensemble, load_config)
...
def test_accuracy_calculator(model, test_iterator, batch_size, device):
    ...
    for batch in test_iterator:
        if batch.review.shape[0] == batch_size:
            ...
            all_predictions.extend(torch.argmax(predictions, dim=1))
            all_target.extend(torch.argmax(target, dim=1))
    performance_matrics = classification_report(all_predictions, all_target, output_dict=True)
    return performance_matrics
...
def run(train_json, test_json, embeddigns, experiment_output_folder, epochs, num_random_search, max_size, device, GS,
        TOKENIZER, PARAMETERS, SAL):
    """
    ...
    for parma_set in GS.random_search(int(num_random_search)):
        ...
        for i in tqdm(range(int(epochs))):
            ...
            model, epoch_loss, performance_matrics = train(ENSEMBLE_MODEL, train_iter, optimizer, criterion, device,
                                                           batch_size)
        ...
        SAL.saver(ENSEMBLE_MODEL, experiment_output_folder,
                  model_performance_metrics={"performance_metrics_train": performance_matrics,
                                             "performance_metrics_test": performance_metrics_test,
                                             "Epoch Loss": epoch_loss}, detailed_params=detailed_params)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--embeddigns',
                        help='Embedding file using prepare_vectors.py or pretrained downloaded vectors.', required=True)
    parser.add_argument('--experiment_output_folder', help='Output folder where all the experiments will be saved.',
                        required=True)
    parser.add_argument('--epochs', help='Max epoch for a single model trainig.', required=True, type=int)
    parser.add_argument('--num_random_search', help='Number of random search to be carried out.', required=True,
                        type=int)
    parser.add_argument('--max_token', help='Max token to be considered for vocab generation.', required=False,
                        default=100000, type=int)
    parser.add_argument('--device', help='CPU or GPU on which computation to be run', required=False, default="gpu")

    args = parser.parse_args()
    run(args.train_json, args.test_json, args.embeddigns, args.experiment_output_folder, args.epochs,
        args.num_random_search, args.max_token, args.device, GS, TOKENIZER, PARAMETERS, SAL)
