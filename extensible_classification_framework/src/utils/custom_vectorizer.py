...
class Vectorizer:
    ...
    def __init__(self,model_path):
        ...
        model_path = /data/extensible_classification_framework/extensible_classification_framework/embedidngs/fastText.model
            ...
            if str(reply).lower() == 'n':
                ...
                for each_word in vocab:
                    if each_word not in unique_tokens:
                        try:
                            file_pointer.write (str("_".join(each_word.replace("\n","").split(" ")))+" "+str(" ".join([str(i) for i in self.model[each_word].tolist()]))+"\n")
